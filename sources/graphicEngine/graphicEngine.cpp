...
void graphicEngine::draw(vector<instance> inst, camera cam,unsigned int shader,lightVec lv,float/* time*/)
{
    ...
    for (unsigned int i=0; i<models.size();i++)
    {
        ...
        {
            if (inst[j].idModel==(int)i+1) //TODO optimiser ça
            {
                glPushMatrix();
...
// 	pthread_mutex_lock(&mutex);
    ...
    glClearColor(1, 0, 0, 1);//On met le fond en rouge car on est censé jamais le voir
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
